...
| RunSilent of string
/// Run `npm run --silent <string>`. Suppresses npm error output and will raise an exception after the script execution instead of failing, useful for CI. See [npm:8821](https://github.com/npm/npm/issues/8821).
| RunTest of string
/// Run `npm test --silent`. Suppresses npm error output and will raise an exception after the script execution instead of failing, useful for CI. See [npm:8821](https://github.com/npm/npm/issues/8821).
| Test
...
let run npmParams =
    ...
    | Some msg ->
        ...
        | RunTest str -> raise (FailedTestsException(msg))
        | Test -> raise (FailedTestsException(msg))
        | _ -> failwith msg
